...
function SignificantClass() {
  ...
  function update() {
    ...
    station_sign_info.forEach(function (item) {
      ...
      update_sort_list.push({
        station_id: item.station_id,
        time: time_value,
        flow: flow_value,
        // routes_num: item.routes_num,
        // DN_result: item.DN_result,
        // result: 0
      });
    });
    // update_sort_list.sort(compare_time);
    // update_sort_list.forEach((item, index) => {
    //   item.time = index;
    // });
    ...
    //   item.flow = index;
    // });
    // update_sort_list.sort(compare_routes_num);
    // update_sort_list.forEach((item, index) => {
    //   item.routes_num = index;
    // });
    // update_sort_list.sort(compare_DN);
    // update_sort_list.forEach((item, index) => {
    //   item.DN_result = index;
    // });
    // update_sort_list.forEach((item) => {
    //   item.result = item.time + item.flow + item.routes_num + item.DN_result;
    // });
    // update_sort_list.sort(compare_result);
    update_sort_list.sort(compare_time);
  ...
  function compare_add_degree(object1, object2) {
    var val1 = object1.add_degree;
    var val2 = object2.add_degree;
    if (val1 < val2) {
      return -1;
    } else if (val1 > val2) {
      return 1;
  ...
  function compare_result(object1, object2) {
    var val1 = object1.result;
    var val2 = object2.result;
    if (val1 < val2) {
      return -1;
    } else if (val1 > val2) {
      return 1;
  ...
  function compare_routes_num(object1, object2) {
    var val1 = object1.routes_num;
    var val2 = object2.routes_num;
    if (val1 < val2) {
      return -1;
    } else if (val1 > val2) {
      return 1;
