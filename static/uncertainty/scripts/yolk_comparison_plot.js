...
function create_yolk_plot(file, go_beginer, back_beginer, gap, offset, station_num) {
        ...
        if (d.station_id == go_beginer) {
          ...
          if (newChain != undefined) {
            ...
            if (station_num != 0) {
              ...
              if (newChain.getLength() == station_num) {
...
function create_node_chain(container, chain_array, station_info, go_beginer, back_beginer, gap, offset) {
  ...
  var xScale = d3.scaleLinear().range([0, 1740]).domain([0, 2600])

  var yScale = d3.scaleLinear().range([150, 2000]).domain([8, 24])
  ...
  var bar = container.append('g').attr('transform', 'translate(50,0)')

  if (offset != undefined)
    bar = container.append('g').attr('transform', 'translate(' + 50 + ',' + (yScale(8 + offset) - yScale(8)) + ')')
  ...
  for (var hour = 8; hour <= 20; hour += gap) {
    ...
    if (location_records_dict[hour + '&' + go_beginer] == undefined) {
...
create_yolk_plot('/static/uncertainty/files/station_run_data2.csv', '11060062', '11000021', 4, undefined, 11)

create_yolk_plot('/static/uncertainty/files/station_run_data_6.csv', '11060062', '11000022', 4, 1, 9)

create_yolk_plot('/static/uncertainty/files/station_run_data_94.csv', '11060062', '11000022', 4, 2, 0)